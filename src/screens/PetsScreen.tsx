  ...
  return (
    <Screen>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>暂无宠物档案，点击“添加宠物”创建。</Text>
        }
        ...
        renderItem={({item}) => {
          ...
          return (
            <Pressable onPress={() => setActivePet(item.id)} style={[styles.card, isActive && styles.activeCard]}>
              <Text style={styles.petName}>{item.name}</Text>
              <Text style={styles.petMeta}>{item.species === 'dog' ? '犬类' : item.species === 'cat' ? '猫类' : '其他'}</Text>
